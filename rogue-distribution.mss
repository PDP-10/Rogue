@Comment{<RELPH.ROGUE>ROGUE-DISTRIBUTION.MSS.9, 12-Nov-87 17:09:56, Edit by RELPH}
@Device(File)
@Begin(Comment)
...
@Center(ECL Rogue@+(*) Distribution)

You may freely copy and use the sources to ECL Rogue (for TOPS-20), as
long as credit is given to the Authors (that's us below).  In
specific, we wish to have our names credited in the version ("v")
command.  Please see the file Rogue.Policy for more details.  In
addition, we would like to receive copies of any new versions of Rogue
created from our sources.  That way we can play it on other machines,
too.  If you would like, we can let you know of any new versions,
also.  Address any Mail to BUG-ROGUE@@ECLC.USC.EDU.
...
Rogue.Configuration@\A generic Rogue configuration file.
...
Rogue.Txt@\An interesting file of (possibly incorrect) hints compiled
by players.

Rogue-Setup.Doc@\A formatted copy of the installation instructions.
Read it to find out more about these files.

Rogue-Init.Exe@\A ready-to-install copy of Rogue.

Rstat-Init.Exe@\A ready-to-install copy of R-Stat.

Rogue-Init.Instructions@\Instructions for creating an init file for
Rogue.
...
@End(Center)
 ...
 @+(*)ROGUE is a trademark of Bell Laboratories (even though ours is
